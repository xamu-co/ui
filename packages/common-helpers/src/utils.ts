...
interface iUseUtils {
	...
	getModifierClasses(values: tProp[], config: { prefix: string }): string[];
	getModifierClasses(
		values: tPropsModifier,
	 ...
	 */
	getPropData: <T extends string>(prop: tProp<T>, index?: number) => T | undefined;
	/**
	 ...
	 */
	createFormData(o: Record<string, any>): FormData;
	/**
	 ...
	 */
	createUrlSearchParams(o: Record<string, any>): URLSearchParams;
}
...
export default function useUtils(options: iPluginOptions = {}): iUseUtils {
	...
	return {
		...
		createFormData(object: Record<string, any>) {
			const formData = new FormData();

			for (const key in object) {
				const isFile =
					Array.isArray(object[key]) &&
					object[key].every((entry: any) => entry instanceof File);
		...
		createUrlSearchParams(object: Record<string, any>) {
